...
def test_login_success_returns_jwt(user_factory):
    ...
    response = client.post(
        reverse("auth-login"), {"email": "demo@demo.com", "password": "Testpwd123"}
    )
...
def test_login_wrong_password(user_factory):
    ...
    client = APIClient()
    response = client.post(
        reverse("auth-login"), {"email": "demo@demo.com", "password": "wrongpass"}
    )
...
def test_login_nonexistent_user():
    client = APIClient()
    response = client.post(
        reverse("auth-login"), {"email": "nouser@demo.com", "password": "Testpwd123"}
    )
    assert response.status_code == 401
    assert "error" in response.data
...
def test_login_inactive_user(user_factory):
    ...
    client = APIClient()
    response = client.post(
        reverse("auth-login"), {"email": "inactive@demo.com", "password": "TestPwd123"}
    )
...
def test_admin_login_success(user_factory):
    ...
    client = APIClient()
    response = client.post(
        reverse("auth-login"), {"email": "admin@demo.com", "password": "AdminPwd123"}
    )
    assert response.status_code == 200
    assert "token" in response.data
...
def test_superuser_wrong_password(user_factory):
    ...
    client = APIClient()
    response = client.post(
        reverse("auth-login"), {"email": "super@demo.com", "password": "badpass"}
    )
    assert response.status_code == 401
    assert "error" in response.data
...
def test_login_unexpected_server_error(monkeypatch):
    ...
    client = APIClient()
    response = client.post(
        reverse("auth-login"), {"email": "demo@demo.com", "password": "pass"}
    )
...
def test_login_blocked_user(user_factory):
    ...
    client = APIClient()
    response = client.post(
        reverse("auth-login"), {"email": "blocked@demo.com", "password": "Block123!"}
    )
    assert response.status_code in (401, 403)
    assert "error" in response.data

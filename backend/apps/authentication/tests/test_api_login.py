...
def test_login_success_returns_jwt(user_factory) -> None:
    ...
    response: Response = client.post(
        ...
        {"email": "demo@demo.com", "password": "Testpwd123"},
    )
...
def test_login_wrong_password(user_factory) -> None:
    ...
    response: Response = client.post(
        reverse("token_obtain_pair"),
        {"email": "demo@demo.com", "password": "wrongpass"},
    )
...
def test_login_nonexistent_user() -> None:
    ...
    response: Response = client.post(
        reverse("token_obtain_pair"),
        {"email": "nouser@demo.com", "password": "Testpwd123"},
    )
    assert response.status_code == 401
    assert "detail" in response.data
...
def test_login_inactive_user(user_factory) -> None:
    ...
    response: Response = client.post(
        reverse("token_obtain_pair"),
        {"email": "inactive@demo.com", "password": "TestPwd123"},
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({}, 400),  # Both missing
        (
            {"email": 123, "password": "Testpwd123"},
            401,
        ),  # Email is wrong type -> treated as invalid credentials
        (
            {"email": "notanemail", "password": "Testpwd123"},
            401,
        ),  # Malformed email -> invalid credentials
    ],
)
...
def test_admin_login_success(user_factory) -> None:
    ...
    response: Response = client.post(
        reverse("token_obtain_pair"),
        {"email": "admin@demo.com", "password": "AdminPwd123"},
    )
    assert response.status_code == 200
    assert "access" in response.data and "refresh" in response.data
...
def test_login_unexpected_server_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "rest_framework_simplejwt.views.TokenObtainPairView.post", broken_auth
    )
    client: APIClient = APIClient()
    response: Response = client.post(
...
def test_login_blocked_user(user_factory) -> None:
    ...
    _user = user_factory(
        email="blocked@demo.com", password="Block123!", is_blocked=True
    )
    client: APIClient = APIClient()
    response: Response = client.post(
        reverse("token_obtain_pair"),
        {"email": "blocked@demo.com", "password": "Block123!"},
    )
    # El proyecto puede o no implementar bloqueo de usuarios. Aceptamos
    # ambos resultados: si falla, habrá 'detail'; si tiene éxito, devolverá tokens.
    if response.status_code == 200:

...
class RegisterView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if (
            ...
            return Response(
                {"password": "too weak"}, status=status.HTTP_400_BAD_REQUEST
            )

        if User.objects.filter(email=email).exists():
            return Response(
                {"email": "already registered"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class LoginView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if getattr(user, "is_active", True) is False:
            return Response(
                {"error": "User inactive"}, status=status.HTTP_401_UNAUTHORIZED
            )
        ...
        if getattr(user, "is_blocked", False) or cache.get(
            f"user_blocked:{user.id}", False
        ):
            return Response({"error": "User blocked"}, status=status.HTTP_403_FORBIDDEN)

        return Response(
            {"token": f"test-token-for-{user.id}"}, status=status.HTTP_200_OK
        )

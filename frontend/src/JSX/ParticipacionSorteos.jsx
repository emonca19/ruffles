...
const TarjetaParticipacion = ({ sorteo, phoneNumber }) => {
  ...
  return (
    <div
      ...
      onClick={() =>
        navigate(`/participacion/detalle/${sorteo.id}`, {
    ...
    >
      <img
        ...
        onError={(e) => {
          ...
          e.target.src =
            "https://placehold.co/300x200/a5b4fc/ffffff?text=Sin+Imagen";
        }}
      />
        ...
        <p className="text-sm text-gray-700 font-semibold">
          ...
          <span className="text-indigo-600 ml-2 font-normal">
            {sorteo.numbers.length > 5
              ? `${sorteo.numbers.slice(0, 5).join(", ")}, y ${
                  sorteo.numbers.length - 5
                } más...`
              : sorteo.numbers.join(", ")}
          </span>
...
export default function ParticipacionSorteos() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!phoneNumber || phoneNumber.length !== 10) {
      setError(
        "Por favor, introduce un número de teléfono de 10 dígitos válido."
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "El servidor devolvió una página HTML (probablemente error 500). Verifica los logs de Django."
          );
        }
        throw jsonError;
      }

      const participacionesAdaptadas = (
    ...
    <div className="main-container">
      ...
      <header className="galeria-header">
        ...
        <p className="text-lg text-gray-600">
          Ingresa tu número de teléfono de 10 dígitos para ver los sorteos en
          los que has participado.
        </p>
      </header>
        ...
        {!hasSearched && !isLoading && (
          <div className="raffle-empty-state">
            <p className="text-lg font-medium">
              Ingresa tu teléfono y presiona "Buscar Boletos" para consultar tu
              estado.
            </p>
          </div>
        )}

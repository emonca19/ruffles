...
const TarjetaEvidencia = ({ evidencia }) => {
  ...
  const formatTickets = (tickets) => {
    ...
    return tickets.length > 6
      ? `${displayTickets}, +${tickets.length - 6} más`
      : displayTickets;
  };
  ...
  return (
    <div
      className={`raffle-card bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden transition-all duration-350 flex flex-col ${
        isPending
          ? "cursor-pointer hover:translate-y-[-6px] hover:shadow-xl hover:border-indigo-600"
          : ""
      }`}
      // ✅ Solución: Pasando el objeto 'evidencia' en el state de navegación al hacer clic en la tarjeta.
      onClick={() =>
        isPending &&
        evidencia.payment_id &&
        navigate(`/verificacion/detalle/${evidencia.payment_id}`, {
          state: { evidencia },
        })
      }
    >
      <div className="relative overflow-hidden">
        <img
          src={
            evidencia.receipt_url ||
            "https://placehold.co/400x200/cccccc/333333?text=Comprobante+No+Disponible"
          }
      ...
      <div className="raffle-info p-5 flex flex-col flex-grow">
        <p className="text-xs text-gray-500 mb-1">
          Compra ID: {evidencia.purchase_id}
        </p>
        <h3
          className={`raffle-name text-lg font-bold text-gray-800 mb-2 leading-snug`}
        >
        ...
        <p className="text-sm text-gray-700 mb-1">
          ...
          {evidencia.customer_name}
        </p>

        <p className="text-sm text-gray-700 mb-3">
          <span className="font-semibold">Boletos:</span>{" "}
          {formatTickets(evidencia.tickets)}
        </p>
        ...
        <div className="mt-auto pt-3 border-t border-gray-100">
          ...
          <p
            className={`text-center text-sm mt-1 font-semibold border py-2 rounded-lg ${getStatusClasses(
              evidencia.status || "unknown"
            )}`}
          >
...
export default function GaleriaEvidencias() {
  ...
  const fetchWithAuth = async (url, options = {}) => {
    ...
    if (!authToken) {
      throw new Error(
        "No autenticado: El token de administrador no está disponible. Por favor, inicie sesión."
      );
    }
  ...
  const fetchEvidencias = React.useCallback(async () => {
    setIsLoading(true);
    setError(null);
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          const rawResponseText = await response
            .text()
            .catch(() => "Detalles no disponibles.");

          if (response.status === 403) {
            throw new Error(
              `Acceso Prohibido (403): Tu usuario no tiene el rol de 'organizer' para acceder a esta página.`
            );
          }
          if (response.status === 401) {
            ...
            navigate("/login");
            throw new Error(
              `Sesión expirada o no autorizada (401). Serás redirigido para iniciar sesión.`
            );
          }
          ...
          try {
            ...
            errorMsg =
              errorData.detail || errorData.non_field_errors?.[0] || errorMsg;
          } catch (e) {
            console.error(
              "Respuesta no JSON:",
  ...
  return (
    ...
    <div className="bg-gray-50 min-h-screen font-sans p-4 md:p-8">
      ...
      <header
        className={`galeria-header text-center p-10 bg-indigo-50 rounded-2xl mx-auto mb-12 max-w-7xl border-2 border-indigo-600 shadow-lg`}
      >
        <h1
          className={`text-indigo-600 font-extrabold text-4xl sm:text-5xl mb-2`}
        >
      ...
      {isLoading ? (
        ...
        <div className="error-message p-6 bg-red-100 border border-red-400 text-red-700 rounded-lg max-w-3xl mx-auto mb-6 text-center shadow-md">
          ...
          {!error.includes("Serás redirigido") &&
        ...
        <div className="raffle-empty-state p-16 text-center text-xl text-gray-700 bg-white border-2 border-dashed border-gray-300 rounded-xl max-w-4xl mx-auto shadow-md">
          ...
          <p className="text-lg text-gray-600">
            No hay comprobantes de pago pendientes de verificación en este
            momento.
          </p>
          <p className="text-gray-500 mt-2">Puedes tomar un descanso.</p>
        </div>
      ) : (
        // Galería de Tarjetas
